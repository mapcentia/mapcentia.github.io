...
function startWfsEdition(layerName, geomField, wfsFilter, single, timeSlice) {
    ...
    $.ajax({
        url: 'http://192.168.33.10/controllers/table/columns/' + layerName,
        async: false,
        dataType: 'json',
        type: 'GET',
        xhrFields: {
    ...
    layer = new OpenLayers.Layer.Vector("vector", {
        ...
        protocol: new OpenLayers.Protocol.WFS.v1_0_0({
            url: "http://192.168.33.10/wfs/" + (subUser ? subUser + "@" + screenName : screenName) + "/" + schema + "/900913" + (timeSlice ? "/" + timeSlice : "") + "?",
            version: "1.0.0",
...
$(document).ready(function () {
    ...
    loadTree = function () {
        ...
        $.ajax({
            url: 'http://192.168.33.10/api/v1/meta/' + screenName + '/' + schema,
            dataType: 'json',
            type: 'GET',
                        ...
                        click: {
                            fn: function (e) {
                                ...
                                $("#ext-" + id).on("click", function () {
                                    ...
                                    Ext.Ajax.request({
                                        url: 'http://192.168.33.10/api/v1/extent/' + screenName + '/' + e.id + '/900913',
                                        method: 'get',
                                        headers: {
