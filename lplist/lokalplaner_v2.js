...
mapcentia_lokalplaner = (function () {
    ...
    var selectControl = {
        ...
        init = function (conf) {
            ...
            store.sql = "select planid,doklink,komnr,objektkode,plantype,plannr,plannavn,anvendelsegenerel as anvgen,anvspec1,datoforsl,(case when planstatus = 'V' then 'Vedtaget' else 'Forslag' END) as planstatus,zonestatus,the_geom from plandatadk.pdk_lokalplan_vedtaget_v where komnr=" + conf.komnr +
            " union select planid,doklink,komnr,objektkode,plantype,plannr,plannavn,anvendelsegenerel as anvgen,anvspec1,datoforsl,(case when planstatus = 'V' then 'Vedtaget' else 'Forslag' END) as planstatus, zonestatus, the_geom from plandatadk.pdk_lokalplan_forslag_v where komnr=" + conf.komnr + " order by planid desc";
            store.load();
