...
Viewer = function () {
    ...
    if (window.gc2Options.leafletDraw) {
        ...
        cloud.map.on('draw:created', function (e) {
            var type = e.layerType, coords = [];
            drawLayer = e.layer;

            if (type === 'marker') {
                var text = markerCounter + "";
                if (text !== null) {
            ...
            $.each(drawnItems._layers, function (i, v) {
                coords.push([v.getLatLng().lng, v.getLatLng().lat]);
            });
            geometryQuery(coords);
        });
                    ...
                    $.each(layers, function (index, value) {
                        ...
                        if (geoType === "RASTER") {
                            ...
                            for (var i = 0; i < coords.length; i++) {
                                /*
                                 "SELECT foo.the_geom,ST_Value(rast, foo.the_geom) As band1, ST_Value(rast, 2, foo.the_geom) As band2, ST_Value(rast, 3, foo.the_geom) As band3 " +
                                 "FROM " + value + " CROSS JOIN (SELECT ST_transform(ST_GeomFromText('POINT(" + coords[i][0] + " " + coords[i][1] + ")',4326)," + srid + ") As the_geom) As foo " +
                                 "WHERE ST_Intersects(rast,the_geom) "
                                 * */
                                unions.push(
                                    "SELECT * FROM (" +
                                    ...
                                    "CROSS JOIN (SELECT ST_Transform(ST_GeomFromText('POINT(" + coords[i][0] + " " + coords[i][1] + ")',4326)," + srid + ") As the_geom) As foo" +
                                    ") as final "
                                );
                            }
